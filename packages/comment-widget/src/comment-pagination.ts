    ...
    return html`
      <ul class="pagination flex items-center gap-4" role="navigation">
        ...
        <li class="inline-flex items-center gap-1 text-sm text-text-3 hover:text-text-1 transition-colors">
          <select name="pagination-value" id="pagination-value" aria-label=${msg('Page')} class="pagination-select appearance-none bg-transparent text-text-1 rounded-base" @change=${(e: Event) => this.gotoPage((e.target as HTMLSelectElement).value)}>
